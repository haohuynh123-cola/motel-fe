...
<template>
  <a-layout class="h-screen overflow-hidden">
    ...
    <a-layout class="flex flex-col h-screen">
      <!-- Header -->
      <a-layout-header
        class="!bg-white px-4 flex justify-between items-center shadow-sm z-10 h-16 flex-shrink-0 border-b border-gray-100"
      >
        <div class="flex items-center gap-6">
          ...
          <a-breadcrumb class="hidden md:flex">
            ...
            <a-breadcrumb-item v-for="crumb in breadcrumbs" :key="crumb.url">
              <router-link
                :to="crumb.url"
                class="font-medium text-gray-500 hover:text-blue-500 transition-colors"
              >
                {{ crumb.label }}
              </router-link>
            </a-breadcrumb-item>
        ...
        <div class="flex items-center gap-4">
          <a-badge :count="5" size="small" :offset="[-2, 2]">
            <div
              class="w-10 h-10 flex items-center justify-center rounded-full hover:bg-gray-100 cursor-pointer transition-colors duration-200 group"
            >
          ...
          <a-dropdown :trigger="['click']">
            <div class="flex items-center cursor-pointer transition-all duration-200 group p-1">
              <a-badge dot color="#52c41a" :offset="[-4, 32]">
                <a-avatar size="large" class="bg-blue-500 shadow-sm border-2 border-white"
                  >A</a-avatar
                >
              </a-badge>
              <div class="ml-3 hidden sm:block">
                <span class="text-sm font-bold text-gray-700 block leading-none"
                  >Quản trị viên</span
                >
              </div>
            </div>
            <template #overlay>
              <a-menu class="min-w-[180px] rounded-xl shadow-xl border-none p-2 mt-2">
                ...
                <a-menu-item
                  key="logout"
                  @click="handleLogout"
                  class="rounded-lg py-2 group hover:bg-red-50"
                >
      ...
      <div class="flex-1 overflow-y-auto bg-[#f8fafc] custom-scrollbar flex flex-col">
        ...
        <a-layout-footer class="!bg-transparent text-center py-4 mt-auto flex-shrink-0">
          <div class="flex flex-col items-center justify-center">
            ...
            <p
              class="text-gray-400/60 text-[10px] font-semibold flex items-center gap-1.5 m-0 uppercase tracking-wider"
            >
